...
class EnumFieldTest < Test::Unit::TestCase
  ...
  context 'allowing nil' do
    context "With an enum that allows nil" do
      setup do
        @possible_values = %w(on off)
        MockedModel.expects(:validates_inclusion_of).with() { |element, options|
          element = :status
          options[:in] = @possible_values
          options[:allow_nil] = true
        }
      end

      should "validate allowing nil" do
        MockedModel.send(:enum_field, :status, @possible_values, :allow_nil => true)
      end
    end

    context "With an enum that does not allow nil" do
      setup do
        @possible_values = %w(on off)
        MockedModel.expects(:validates_inclusion_of).with() { |element, options|
          element = :status
      ...
      should "validate not allowing nil" do
        MockedModel.send(:enum_field, :status, @possible_values, :allow_nil => false)
      end
    end
  end
end
